...
| FuseSoC | 2.4.7 project-local environment (`.tools/`) |
| PDK | `ihp-sg13g2` |
...
`make setup` creates `.tools/fusesoc-2.4.7`, so it does not use an older
FuseSoC that may be present in nixpkgs. Version 2.4.3 or newer is mandatory
because the pinned Ibex/lowRISC core files use the CAPI2 `mapping` property.
The environment also installs `packaging`, which is required by Ibex's
`check_tool_requirements.py` pre-build hook. `make lint` prepends the project
environment to `PATH`, ensuring that hook uses the matching Python interpreter.
The script then clones Ibex at the pinned commit, registers the local FuseSoC
...
| `must not contain {'mapping'}` | an old FuseSoC was invoked; use `make setup`, which pins 2.4.7 under `.tools/` |
| `No module named 'packaging'` | refresh the project environment with `make fusesoc-refresh`, then rerun `make lint` |
| SRAM instance not found | hierarchy must remain `i_soc.u_imem.u_sram` and `i_soc.u_dmem.u_sram` in RTL, YAML and PDN Tcl |
| `SRAM_1P_behavioral_bm_bist` missing | use the included `ip/ihp_sram/verilog/*.bb.v` as the macro `vh`; do not use the PDK simulation wrapper for lint/synthesis |
| FuseSoC reports an old `/workspace/scratch/...` library | use the included project-local `fusesoc.conf`; current scripts pass checkout paths with `--cores-root` and ignore stale global registrations |
| SDC reports `invalid command name "remove_from_collection"` | use the included OpenSTA-compatible `chip_top.sdc`, which constrains explicit input/output pad-port collections |
| `add_global_connections` makes 0 connections for a power pad | keep all four power-pad instances with the included `(* keep *)` attributes so synthesis and OpenDB retain them |

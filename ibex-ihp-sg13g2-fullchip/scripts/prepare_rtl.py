...
version=subprocess.check_output([FUSESOC,'--version'],text=True).strip()
print(f'Using FuseSoC {version} from {FUSESOC}')
run(FUSESOC,*FUSESOC_ARGS,'run','--target=lint','--setup','local:ibex:ihp_fullchip:1.0')
...
for f in data['files']:
    ...
    seen.add(rp)
    # Lint-only declarations for IHP hard macros must not enter LibreLane.
    if p.name == 'ihp_tech_stubs.sv': continue
